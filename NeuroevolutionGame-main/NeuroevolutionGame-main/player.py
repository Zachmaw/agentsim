...
class Player(pygame.sprite.Sprite):
    def __init__(self, game_mode):
        ...
        if self.game_mode == "Neuroevolution":
            ...
            self.penalty = 0  # Initial Cost

            layer_sizes = [8, 18, 2]  # TODO (Design your architecture here by changing the values)
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        if len(obstacles)>=3:
            ...
            x2 = 2*player_x/3
            x3 = player_x/2
            y1 = 2*screen_height/3
            y2 = screen_height/2
            y3 = screen_height/3

        sum = sqrt(x1**2 + y1**2 + x2**2 + y2**2 + x3**2 + y3**2 + player_x**2 + player_y**2)
        #input = [x1/sum, x2/sum, x3/sum, y1/sum, y2/sum, y3/sum, player_x/sum, player_y/sum]
        input = [x1, x2, x3, y1, y2, y3, player_x, player_y]
