...
def display_time():
    gametime = int((pygame.time.get_ticks() - start_time) / 100)
    score_surf = game_font.render(f"Time: {gametime}", False, (64, 64, 64))
...
def create_players(mode, player_list=None):
    ...
    players = pygame.sprite.Group()### by defining a new sprite group each time?! just init the group once and empty it when you have to?!
    if mode == "Manual":
...
def update_fitness():
    for player in players:
        player.fitness = current_runtime - player.penalty
...
if __name__ == '__main__':
    ...
    # Timer### swap to a random method rather than timing
    ...
    while True:
        ...
        if game_active:
            ...
            players.draw(screen)
            current_runtime = display_time()
            if game_mode == "Neuroevolution":
                display_generation()
                update_fitness()
            if current_runtime > best_score:
                best_score = current_runtime
